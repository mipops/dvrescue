...
return_value Help(ostream& Out, const char* Name, bool Full)
{
    ...
    Out << "\n"
        ...
        "\n"
        "    --rewind-capture\n"
        "        Also capture frames during reverse playback (rewind) and\n"
        "        use them as an additional data source for error correction.\n"
        "        Captures data in three phases: reverse play, direction\n"
        "        change settling, and forward approach. Alternates reverse\n"
        "        speed between -1.0x and -0.5x across passes for diverse\n"
        "        head-tape contact. Implies --rewind and --rewind-overshoot 30.\n"
        "\n"
        "    --rewind-overshoot value\n"
        "        Number of frames to rewind beyond the error region, giving\n"
        "        the transport time to stabilize before re-reading. Default 0\n"
        "        (or 30 when used with --rewind-capture).\n"
        "\n"
