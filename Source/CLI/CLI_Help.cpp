...
return_value Help(ostream& Out, const char* Name, bool Full)
{
    ...
    Out << "\n"
        ...
        "\n"
        "    --errors-only | -e\n"
        "        Only show frames that contain errors in the XML output.\n"
        "\n"
        "    -y\n"
        "        Answer yes to all prompts (e.g. overwrite existing files).\n"
        "\n"
        "    -n\n"
        "        Answer no to all prompts (e.g. do not overwrite existing files).\n"
        "\n"
        ...
        "\n"
        "    --rewind-threshold value\n"
        "        Minimum error density percentage in an error region to trigger\n"
        "        a rewind. If the percentage of bad blocks in the region is below\n"
        "        this threshold, the rewind is skipped. Default 0 (always rewind).\n"
        "\n"
        "    --rewind-margin value\n"
        "        Number of extra frames to include around each error region when\n"
        "        rewinding, to cover adjacent marginal-quality frames. Default 0.\n"
        "\n"
        "    --rewind-speeds value\n"
        "        Comma-separated list of playback speeds for rewind passes.\n"
        "        Each pass cycles through the list. Example: -1.0,-0.5,-0.33\n"
        "        Default: -1.0,-0.5 (alternating normal and slow reverse).\n"
        "\n"
        ...
        "        1080i5994  Set video mode to 1080i 59.94 (HDV NTSC).\n"
        "        1080i50    Set video mode to 1080i 50 (HDV PAL).\n"
        "        720p5994   Set video mode to 720p 59.94 (HDV 720p NTSC).\n"
        "        720p50     Set video mode to 720p 50 (HDV 720p PAL).\n"
        "\n"
        ...
        "        Select DeckLink capture pixel format.\n"
        "        value may be:\n"
        "        uyvy        8-bit YUV (UYVY 4:2:2). Better for BetacamSX.\n"
        "        v210        10-bit YUV (v210 4:2:2, default).\n"
        "        argb        8-bit ARGB.\n"
        "        bgra        8-bit BGRA.\n"
        "        r210        10-bit RGB.\n"
        "\n"
        "    --decklink-audio-channels <count>\n"
        "        Select number of audio channels for DeckLink capture.\n"
        "        value may be: 2 (default), 8, or 16.\n"
        "        Some tape formats have more than 2 audio channels.\n"
        "\n"
        #endif
        "If no output file name is provided, XML output is displayed on console output.\n"
        "\n"
        "Examples:\n"
        "    Analyze a DV file and display XML on console:\n"
        "        dvrescue input.dv\n"
        "\n"
        "    Save analysis as XML:\n"
        "        dvrescue input.dv -x output.dvrescue.xml\n"
        "\n"
        "    Merge multiple DV captures of the same tape into one file:\n"
        "        dvrescue input1.dv input2.dv input3.dv -m merged.dv\n"
        "\n"
        "    Merge multiple HDV captures of the same tape (.m2t):\n"
        "        dvrescue input1.m2t input2.m2t -m merged.m2t\n"
        "\n"
        "    Capture from a connected DV device:\n"
        "        dvrescue device://0 -m capture.dv -x capture.dvrescue.xml\n"
        "\n"
        "    Capture with automatic rewind on errors (3 retries):\n"
        "        dvrescue device://0 -m capture.dv --rewind-count 3\n"
        "\n"
        "    Capture with rewind and reverse-playback error correction:\n"
        "        dvrescue device://0 -m capture.dv --rewind-count 3 --rewind-capture\n"
        "\n"
        ...
        "\n"
        "    Capture HDV via DeckLink (1080i NTSC):\n"
        "        dvrescue device://0 --decklink-video-mode 1080i5994 -m capture.m2t\n"
        "\n"
